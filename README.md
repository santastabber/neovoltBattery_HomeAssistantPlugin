...
- **Off-grid Mode Binary Sensor**: The integration exposes a binary sensor for ByteWatt/Neovolt off-grid or backup mode status. It reads the `upsModel` value from the latest power data response after resolving the real system serial number, matching the status shown in the ByteWatt/Neovolt web UI.
